...
function watchTrackingResolution(callback, { distance = 0, heading = 0, time = 0, namespace, prefix, deleteOnExit = true, highPosAcc = 0, lowPosAcc = 0, highNegAcc = 0, lowNegAcc = 0 }) {
    ...
    if (!(!heading && !time && !distance))
        Utils.OSExecute(`apx-tracking set  --namespace="${name}" --heading=${heading} --time=${time} --distance=${distance} --acc-thresholds=${highPosAcc},${lowPosAcc},${highNegAcc},${lowNegAcc}`); // Adde accel variables
    var handler = function (channel, gps) {
