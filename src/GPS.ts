...
function watchTrackingResolution(callback, { distance = 0, heading = 0, time = 0, namespace, prefix, deleteOnExit = true, posAcc = 0, negAcc = 0, posUnits = "", negUnits = "" }) {
	...
	posUnits = posUnits || "";
	negUnits = negUnits || "";
	if (!(!heading && !time && !distance)) Utils.OSExecute(`apx-tracking set  --namespace="${name}" --heading=${heading} --time=${time} --distance=${distance} --pacc=${posAcc}${posUnits} --nacc=${negAcc}${negUnits}`); // Adde accel variables
	var handler = function (channel, gps) {
